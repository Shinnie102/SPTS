...
class ReportService
{
    ...
    public function getReportPageData(int $classSectionId, int $lecturerId): array
    {
     ...
     */
    public function getReportData(int $classSectionId, int $lecturerId): array
    {
        ...
        try {
            $currentClass = ClassSection::where('class_section_id', $classSectionId)
                ->where('lecturer_id', $lecturerId)
                ->first();
            ...
            foreach ($students as $s) {
                ...
                $pf = $this->calculationService->passFail($finalRounded);
     ...
     * Returns tuple: [statusCode, payload]
     */
    public function getStudentDetail(int $classId, int $studentId, int $lecturerId): array
    {
        ...
        $finalResult = $this->calculationService->calculateFinalScore($componentList, $scoreMap);
        $finalRounded = $finalResult['rounded'] ?? null;
        $finalStatus = $finalResult['status'] ?? $this->calculationService->evaluateScore($finalRounded);

        $componentsForJson = array_map(function ($c) use ($scoreMap) {
            $componentId = (int) ($c['component_id'] ?? 0);
            $raw = array_key_exists($componentId, $scoreMap) ? $scoreMap[$componentId] : null;
            $rounded = $this->calculationService->roundScore($raw);
            return [
    ...
    public function getExportReportViewData(int $classSectionId, int $lecturerId): array
    {
        ClassSection::where('class_section_id', $classSectionId)
            ->where('lecturer_id', $lecturerId)
            ->firstOrFail();
